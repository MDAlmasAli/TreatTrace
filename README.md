...
- Appointment tile badge now shows "Dr. Name — DD Mon YYYY" instead of static "Linked Prescription"
- Fetches prescription data at runtime via `PrescriptionService.fetchOne`; falls back to "Linked Prescription" while loading or if deleted

**v0.51 — Fix: Appointment sort order in doctor patient detail**

- Upcoming (scheduled) appointments now appear first, sorted ascending by date (soonest first)
- Past/completed appointments appear below, sorted descending (most recent first)
...
- Test report detail screen now shows an "Ordered By" section when a doctor was linked via the picker
- Fetches doctor's full name, specialty, and hospital at runtime from `doctor_verifications`
- Fallback to manually entered `doctorName`/`hospital` fields for older records
- "Test Information" card cleaned up (now only shows test name and date)

**v0.48 — Fix: Today's Appointments count matches Today tab**

- `countTodayForCurrentDoctor()` now only counts `scheduled` appointments for today
- Previously it counted all statuses (completed/cancelled included), causing the home screen count to be higher than what Today's Schedule actually showed
